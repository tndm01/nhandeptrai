(function (app) {
    ...
    applicationRoleListController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', '$stateParams'];

    function applicationRoleListController($scope, apiService, notificationService, $ngBootbox, $filter, $stateParams) {
        $scope.loading = true;
        $scope.data = [];
        $scope.page = 0;
        $scope.pageCount = 0;
        ...
        function loadRolesName() {
            apiService.get('api/applicationRole/getRoleName', function (response) {
                $scope.nameRoles = response.data;
                var name = [];
                $.each(response.data, function (i, item) {
                    name.push(item.Name);
                });
                $scope.nameRoles = name;
            }, function (response) {
...
})(angular.module('nnshop.application_roles'));